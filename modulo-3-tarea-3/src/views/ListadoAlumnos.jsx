...
import { subscribeStudents, addNewStudent, updateStudent, mergeUpdateStudent, deleteStudent } from "../services/apiFirebase";
    ...
    <main className="alumnos-main">
      <section className="alumnos-list">
        ...
        <div className="auth-box">
          {user && (<p>Sesión iniciada como <strong>{user.email}</strong></p>)}
        </div>
        ...
        <form ref={formRef} className="alumnos-form" onSubmit={handleSubmit}>
          ...
          <div className="form-buttons">
            ...
            {(editingId || formData.name || formData.course || formData.age !== "" || formData.photo !== "") && (
                <button type="button" onClick={resetForm} disabled={loading} className="btn-secondary">Limpiar</button>
            )}
            </div>
        </form>
        ...
        <div className="students-grid">
          {students.length === 0 ? (
            <p style={{ opacity: 0.8 }}>No hay alumnos todavía? Agrega uno con el formulario.</p>
          ) : (
            students.map((s) => (
              <article className="student-card" key={s.id}>
                ...
                <div className="card-buttons">
                  <button onClick={() => handleEdit(s)} disabled={!user || loading}>Editar</button>
                  <button onClick={() => handleDelete(s.id)} disabled={!user || loading}>Eliminar</button>
                </div>
              </article>
            ))
          )}
